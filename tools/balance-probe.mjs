 ...
 */
{
  ...
  /**
   * (a) 안전망 — 어려워하는 아이는 낮은 단계에 머물러야 한다.
   *
   * 🔴 예전엔 **정답 60% 이하 전부**에 "0.5단계 이하"를 걸었다. 그런데 60%는
   *    열 문제 중 여섯을 맞히는 아이다 — 어려워하는 아이가 아니라 **배우는 중인 아이**다.
   *    그 대역까지 0단계에 묶어 두니 30판 내내 패배율 4%(사실상 못 지는 게임)가 되었고,
   *    실사용자가 세 번에 걸쳐 "너무 쉽다"를 보고했다.
   *    그래서 대역을 갈랐다: **40% 이하 = 안전망(0.5단계 이하, 종전 그대로),
   *    60% = 완만한 도전(1.2단계 이하)**. 40% 아이의 실측치는 이 변경 전후 모두
   *    0.0단계 · 패배율 38% 로 **한 칸도 움직이지 않았다** — 안전망은 그대로다.
   */
  for (const r of rows.filter((x) => x.s <= 0.6)) {
    const cap = r.s <= 0.4 ? 0.5 : 1.2;
    if (r.tier > cap) fails.push(`G8-a 정답${Math.round(r.s * 100)}% 아이가 ${r.tier.toFixed(1)}단계까지 올라감 (>${cap}) — 안전망 이탈`);
  }
  ...
  for (const r of rows.filter((x) => x.s >= 0.75)) {
    ...
    /**
     * 🔴 상한이 45% 였다 → 55%. 실사용자가 **세 번** "너무 쉽다"를 보고했고, 45% 상한은
     *    그 상태를 초록불로 통과시키고 있었다. 반은 지는 정도가 이 게임이 노리는 자리다.
     * 🔴 그래도 상한을 없애지는 않는다 — 이 게이트가 잡아야 하는 건 "적정 도전"이 아니라
     *    **되먹임이 폭주해 아무도 못 이기는 상태**다. 그리고 지는 게 벌이 아니라 재도전이
     *    되는지는 G9(재도전 루프)가 따로 잰다. 둘 다 통과해야 난이도 변경이 유효하다.
     */
    if (r.lost > 0.55) fails.push(`G8-c 정답${p}% 아이 패배율 ${Math.round(r.lost * 100)}% (>55%) — 벽`);
  }
  /**
