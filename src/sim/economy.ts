...
// ── 판 보상(먹물) ────────────────────────────────────────────────────────
/**
 * 한 판을 치르면 **맞힌 문제 수만큼** 먹물을 받는다. 이기든 지든.
 *
 * 🔴 **없어서 게임이 막혀 있었다.** 2026-08-11 전수 조사: 먹물이 늘어나는 곳이
 *    `src/ui/screens.ts` 의 관문(gate) 한 곳뿐이었고, 지급 조건이 "별이 이전 기록보다
 *    나아진 만큼"이었다. 그래서
 *      · 지면 관문에 못 가므로 **보상 0** (`main.ts` 의 win 분기)
 *      · 이미 3별인 판을 다시 이겨도 `max(0, 60-60)` = **0**
 *    즉 **막힌 아이에게는 수입 경로가 존재하지 않았다.** 전력을 올리려면 먹물이 필요한데
 *    먹물을 벌려면 이겨야 하고, 이기려면 전력이 필요하다. 순환이다.
 *    사용자가 요구한 재미("못 깨도 계속 도전하고 업그레이드하면서 못 깨던 판을 깨는 것")가
 *    **경제적으로 불가능한 상태**였고, 2026-08-07 에 패배율을 40~50%로 올리면서 더 아파졌다.
 *
 * 🔴 그리고 `stages.ts` 머리말은 "먹물은 정답에서만 나오므로 더 많이 맞힌 아이가 더 멀리
 *    간다"고 선언한다. 실제로는 별에서 나왔다 — 선언과 구현이 어긋나 있었다. 이 함수가 맞춘다.
 *
 * 🔴 **파밍 걱정은 여기서 뒤집힌다.** 보통 게임에서 같은 판을 반복해 자원을 버는 건 문제지만,
 *    이 게임에서 그 행위는 **연산 문제를 더 푸는 것**이다. 반복이 곧 학습이라 막을 이유가 없다.
 *    다만 시간당 효율에 상한은 둔다(`MATCH_INK_CAP`) — 한 판을 무한정 늘려 버는 건 막는다.
 */
export const INK_PER_CORRECT = 3;
/** 한 판에서 받을 수 있는 먹물 상한 — 판을 길게 끌어 버는 것을 막는다 */
export const MATCH_INK_CAP = 180;
/** 뒤 판일수록 문항이 어려우니 같은 정답 수라도 더 준다(별 보상의 scale 과 같은 결) */
const MATCH_INK_SLOPE = 0.06;

export function matchInk(correct: number, stage: number): number {
