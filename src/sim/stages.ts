 ...
 */
/**
 * 🔴 **지분은 체력만 고정한다 — 공격력은 아무도 안 보고 있었다(2026-08-20 실측).**
 *    `e_minus`(빼기까마귀)는 공격력 72 로 보스를 뺀 최고치이고 `e_mul`(24)의 3배인데
 *    지분은 0.10 이었다. 단계 배율이 공격력에 ×105 로 곱해지는 상단에서는 이 한 종이
 *    난이도 곡선을 통째로 꺾는다 — 이 괴수가 사는 ST12~20 구간만 뒤보다 어려웠다:
 *      (정답95% 어른, 단계 16) ST14 29% · ST15 24% · ST16 62% · ST18 38% · **ST21 90%**
 *    지분을 0.05 로 내리면 같은 구간이 71% · 100% · 90% · 100% 로 펴진다.
 *    바로 위 주석이 "특기 있는 종은 지분을 낮게 준다"고 적어 둔 그 규칙인데,
 *    **높은 공격력도 '특기'라는 것**이 빠져 있었다.
 */
